...
import top.continew.admin.ai.model.resp.ChatCompletionResponse;
        ...
        return webClient.post()
                ...
                .flatMap(data -> {
                    if ( "[DONE]".equals(data) ) {
                        ...
                        // 如果是结束标志
                        return Flux.just(ServerSentEvent.builder(jsonObject).event("done").id(IdUtil.fastSimpleUUID()).build());
                    }
                    // 解析响应内容
                    try {
                        ...
                        sb.append(content);
                        return Flux.just(ServerSentEvent.builder(ModelMessageUtils.convertModelChatResponse(taskId, content)).event("add").id(IdUtil.fastSimpleUUID()).build());
                    } catch (Exception e) {
                        // 如果解析失败
                        return Flux.just(ServerSentEvent.builder(ModelMessageUtils.convertModelChatResponse(IdUtil.fastSimpleUUID(), "服务异常请联系管理员")).event("error").build());
                    }
                })
                .onErrorResume(e -> {
