...
public class AliPayController {
    ...
    @Operation(summary = "统一收单下单并支付页面接口", description = "统一收单下单并支付页面接口")
    @PostMapping("/trade/page/pay/{orderNo}")
    public R<Object> tradePagePay(@PathVariable String orderNo) {
        log.info("统一收单下单并支付页面接口");
        //发起交易请求后，会返回一个form表单格式的字符串（要有前端渲染）
        String formStr = aliPayService.tradeCreate(orderNo);
    ...
    public String tradeNotify(@RequestParam Map<String, String> params) {
        try {
            ...
            boolean signVerified = AlipaySignature.rsaCheckV1(params, config
                .getProperty("alipay.alipay-public-key"), AlipayConstants.CHARSET_UTF8, AlipayConstants.SIGN_TYPE_RSA2);
            ...
            OrderInfoDO order = orderInfoService.getOrderByOrderNo(outTradeNo);
            //对订单对象进行判断
            if (order == null) {
                log.error("订单不存在......");
                return result;
     ...
     * @return
     */
    @Operation(summary = "商户查询订单", description = "商户查询订单")
    @GetMapping("/trade/query/{orderNo}")
    public R<Object> queryOrder(@PathVariable String orderNo) {
     ...
     * @return
     */
    @Operation(summary = "商户退款接口", description = "商户退款接口")
     ...
     * @return 返回查询的结果
     */
    @Operation(summary = "查询退款", description = "查询退款")
    @GetMapping("/trade/fastpay/refund/{orderNo}")
    public R<Object> queryRefunds(@PathVariable("orderNo") String orderNo) {
        log.info("查询退款.......");
        //执行退款查询并接收返回的字符串结果
     ...
     * @return 返回账单的url地址
     */
    @Operation(summary = "获取账单url", description = "获取账单url")
    @GetMapping("/bill/downloadurl/query/{billDate}/{type}")
    public R<Object> queryTradeBill(@PathVariable String billDate, @PathVariable String type) {
        log.info("获取账单的url地址");
        //获取账单的url地址
